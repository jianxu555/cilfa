    ...
    for source_index in source_indexs:
        ...
        for epo in range(300):
            ...
            for batch in tqdm(dataloader):
                ...
                p_style_conv_out, p_style_avg_out = f_extractor(style_imgs)  # [bs, 512, 8, 8]  [bs, 2048],

                p_semi_conv_out, p_semi_avg_out = f_extractor(semi_imgs)  # [bs, 512, 8, 8]  [bs, 2048],
                ...
                # L_cls_init =  cross_ent(clsfier(p_style_yes_avg_out), label) + F.kl_div( F.log_softmax(pred_no_out, dim=-1),
                #                                                                             uniform_target,
                #                                                                             reduction="batchmean"
                #                                                                         )
                # L_cls_init =  cross_ent(clsfier(p_style_yes_avg_out), label) + F.kl_div( F.log_softmax(pred_no_out, dim=-1),
                ...
                #                                                                         )
                L_cls_init = cross_ent(clsfier(torch.cat((a_yes_avg_out, p_style_yes_avg_out, p_semi_yes_avg_out), 0)),
                                       torch.cat((label, label, label), 0)) + F.kl_div(F.log_softmax(pred_no_out, dim=-1),
                                                                                       uniform_target,
                                                                                       reduction="batchmean"
                                                                                       )
                ...
                combined_labels = label.repeat(args.GEN_TIMES * 2)
                ...
                combined_feat = torch.cat([yes_feat_expanded, no_feat_expanded],
                                          dim=-1)  # [N_c, N_b, feat_dim_c+feat_dim_b]
                combined_feat = combined_feat.reshape(-1, combined_feat.shape[-1])  # 展平为[N_c*N_b, total_dim]

                all_labels = label.repeat(all_yes_feat.shape[0] // label.shape[0]).unsqueeze(1).repeat(1,
                                                                                                       N_b).reshape(
                    -1)
                ...
                pred_causal_expanded = pred_causal.unsqueeze(1).repeat(1, N_b, 1).reshape(-1, pred_causal.shape[
                    -1])
                ...
                ################### L_int
                L_int = cross_ent(pred_intervened_shuffled, all_labels_shuffled) + kl_divergence(pred_intervened_shuffled,
                                                                                                 pred_causal_expanded_shuffled)
